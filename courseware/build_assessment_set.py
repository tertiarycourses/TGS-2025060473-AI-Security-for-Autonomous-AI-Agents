...
# ================================================================== CASE STUDY content
# The Case Study is a reflection on the activities the learner completed during the
# day. There is one task per Learning Outcome, each with TWO questions; together
# they cover all five accredited Ability statements. No fictional company or data
# tables are used — the "evidence" is the learner's own observations from the
# in-class activities.
CS_ORG = "Your own activity work"

CS_SCENARIO_INTRO = (
    "This Case Study is based on the eight hands-on activities you completed today. "
...
CS_SCENARIO_BODY = [
    ...
    (None, "bullets", [
        ...
        "mock Excel marketing data and to build a 10-slide AI-security PowerPoint that you then "
        "redesigned from an uploaded image template, then added a tool/skill and re-ran the tasks.",
...
# The Case Study uses no data tables. These are kept empty so the renderer stays happy.
CS_TABLE_A = None
CS_TABLE_A2 = None
CS_TABLE_B = None
CS_TABLE_SEG = None
...
CS_Q = [
    (1, "LO1", ["A3", "A4"], 20,
     ...
     ["(a) Using your Activity 1 experience with the AI agent on WhatsApp, explain in your own "
      "words the difference between generative AI, agentic AI and an AI agent. (10 marks)",
      "(b) Give ONE example from today where the AI was useful and ONE where you would not "
      "fully trust it, and say why. (10 marks)"],
     22),

    (2, "LO2", ["A5"], 24,
     "Reflect on Activities 2-4, where you compared weak and strong prompts on the same data and "
     "then added a tool or skill.",
     ["(a) Write out one WEAK prompt and one STRONG prompt you used today for the same task, "
      "and describe how the two outputs differed. (14 marks)",
      "(b) Explain which prompt-engineering principles made the stronger prompt work (for "
      "example role, task, context, format, constraints). (10 marks)"],
     22),
    ...
    (3, "LO3", ["A1", "A2"], 26,
     ...
     ["(a) From Activity 7, describe what the leaky chatbot exposed and how the guarded chatbot "
      "stopped the same attack. Name at least two guardrail layers you saw. (14 marks)",
      "(b) From your Activity 5 policy and your Activity 6 coaching, explain who should be "
      "accountable when an AI agent handles data or affects a person's job, and why it is never "
      "the AI itself. (12 marks)"],
     22),
]
...
CS_ANSWERS = {
    1: dict(
        ...
        blocks=[
            ("(a) Generative vs agentic vs agent — 10 marks", [
                "Generative AI = makes content (a step you run). (about 4 marks)",
                "Agentic AI = the loop that makes the AI pursue a goal — plan, act, verify, "
                "repeat. (about 4 marks)",
            ...
            ("(b) Useful vs not-trusted example — 10 marks", [
                "One concrete 'useful' example from today's activities (e.g. a good draft "
                "reply, a clean data summary, the slides it built) — 5 marks.",
                "One concrete 'would not trust' example with a reason (e.g. it stated a fact it "
                "could not know; it could see personal data; a confident but wrong answer) — "
                "5 marks.",
                "Both examples must come from what the candidate actually did or saw today — a "
                "generic textbook example caps each half at 2.",
            ]),
        ],
    ...
    2: dict(
        ...
        blocks=[
            ("(a) Weak vs strong prompt and the difference — 14 marks", [
                "A real weak prompt and a real strong prompt for the SAME task — 6 marks.",
                "A clear description of how the outputs differed (the strong one was usable, "
                "structured, on-format; the weak one vague) — 8 marks.",
                "Example: weak 'analyse this excel' vs strong 'As a marketing analyst, list the "
                "top 3 channels by ROI with one action each'.",
            ]),
            ("(b) Principles that made it work — 10 marks", [
                ...
                "About 2-3 marks each, up to 10.",
            ]),
        ],
        guidance="Reward a genuine before/after from the candidate's own prompts. If they cannot "
                 "produce two contrasting prompts they did today, (a) caps at 6. Satisfactory "
    ...
    3: dict(
        ...
        blocks=[
            ("(a) Leaky vs guarded chatbot — 14 marks", [
                ...
                "input guard, hardened prompt, output guard (redaction) — 8 marks (4 marks per "
                "correctly described layer, up to 8).",
            ]),
            ("(b) Accountability — 12 marks", [
                "A named human role is accountable (data owner, agent owner, manager) — 6 marks.",
                "Why it is never the AI: the AI is a tool with no legal responsibility; the "
                "organisation answers for it (accept the Moffatt v Air Canada point) — 6 marks.",
                "Credit answers grounded in the candidate's own Activity 5 policy roles or their "
                "Activity 6 coaching conversation.",
            ]),
        ],
        guidance="Reward reflection grounded in what the candidate saw in Activities 5-8. The "
                 "accountability point (never the AI) must appear for full marks in (b). "
                 "Satisfactory when (a) and (b) are both addressed with real observations.",
    ),
}
...
CS_INSTRUCTIONS = [
    ...
    "3. Answer ALL THREE (3) tasks. Each task has TWO questions, (a) and (b). Every question "
    "is open-ended — there are no multiple-choice options.",
    "4. A total of 25 minutes is allowed for this Case Study.",
...
def coverage_table(d, kind):
    ...
    if kind == "WA":
        ...
        line(d, "Every accredited Ability (A) statement of TSC " + TSC_CODE + " is assessed "
                "across the three Case Study tasks, one task per Learning Outcome with two "
                "questions each. All five A statements are covered; none is unassessed.",
             size=10, color=GREY, after=8)
    ...
    if kind == "WA":
        ...
        line(d, "Coverage confirmation: A1 ✓ (T3a) · A2 ✓ (T3b) · A3 ✓ (T1a) · "
                "A4 ✓ (T1b) · A5 ✓ (T2a, T2b) — 5 of 5 ability statements assessed across "
                "3 tasks, one per Learning Outcome, two questions per task.",
             bold=True, size=10, color=DARK, after=4)
...
def scenario_pages(d, with_role=True):
    """Renders the Case Study reflection context. Starts on the CURRENT page."""
    line(d, "SECTION D: YOUR ACTIVITY WORK TODAY", bold=True, size=13, color=BRAND, after=3)
    line(d, "Case Study — reflection on the activities you completed",
    ...
    # Optional data tables — only rendered if present (the reflection Case Study has none).
...
def build_cs_paper():
    ...
    admin_page(d, "Case Study", "25 minutes", 70, 3, "tasks",
               CS_INSTRUCTIONS)                                           # page 2

    scenario_pages(d)                                                     # page 3 onwards
    page_break(d)

    line(d, "SECTION E: CASE STUDY TASKS", bold=True, size=13, color=BRAND, after=3)
...
def build_cs_answers():
    ...
    line(d, "NOTE TO ASSESSOR", bold=True, size=12, color=BRAND, after=4)
    for t in [
        "This is a REFLECTION-BASED case study. The candidate answers THREE open-ended tasks "
        "(two questions each) about the eight activities they completed during the course, one "
        "task per Learning Outcome. Together the three tasks cover all five accredited Ability "
