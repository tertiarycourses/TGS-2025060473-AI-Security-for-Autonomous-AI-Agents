...
ACTIVITIES = [
    ...
    },
    {
        ...
        "duration": "25 minutes",
        "tool": "Your Hermes agent connected to a MiniMax model, plus the mock marketing Excel file in the activity pack.",
        "evidence": "SIM — the marketing data is fictional teaching data.",
        "steps": [
            "Open the mock marketing Excel file supplied in the activity pack and skim the columns.",
            "Send the BAD prompt from the prompt card to your agent (for example, 'look at this excel') and read the response.",
            "Send the GOOD prompt from the prompt card (a clear role, task and format) and read the response.",
            "Compare the two answers: which one could you actually act on?",
            "Ask one follow-up to refine the good answer into a three-bullet summary with one action each.",
        ],
        "produce": ["The two responses (bad-prompt and good-prompt) side by side",
                    "A three-bullet insight summary you could hand to a manager"],
        "acceptance": "You ran the same data through a weak and a strong prompt and can explain, in one sentence, why the stronger prompt produced a more useful answer.",
    },
    {
        "n": 3, "topic": 2,
        "title": "Build an Animated PowerPoint with the Agent",
        "folder": "activity-3-ppt-builder",
        "duration": "25 minutes",
        "tool": "Your Hermes + MiniMax agent, the script and prompts in the activity pack, and a PowerPoint template from elements.envato.com/presentation-templates/compatible-with-powerpoint.",
        "evidence": "SIM — the script content is fictional teaching material.",
        "steps": [
            "Open the supplied script and the list of prompts in the activity pack.",
            "Choose an Envato PowerPoint-compatible template as your starting style.",
            "Send the VAGUE prompt ('make a ppt') and note how little you can use.",
            "Send the DETAILED prompt (template, number of slides, animation on titles, a summary slide) from the prompt card.",
            "Ask the agent to adjust one thing — for example, shorten a slide or add a closing call to action.",
        ],
        "produce": ["A short slide outline or deck built from the detailed prompt",
                    "A note on the single change that most improved the result"],
        "acceptance": "You produced a usable slide outline from a detailed prompt and can point to what made it better than the vague request.",
    },
    {
        "n": 4, "topic": 2,
        ...
        "evidence": "SIM — reuse the same fictional Excel and script from Activities 2 and 3.",
        "steps": [
    ...
    },
    {
        ...
        "duration": "30 minutes",
        "tool": "The sample AI Data Governance Policy and adaptation prompts in the activity pack; any AI assistant you used today.",
        "evidence": "SIM — write about a fictional or generic team, not real confidential systems.",
        "steps": [
            "Open the sample AI Data Governance Policy in the activity pack.",
            "Read the five sections: Scope, Roles, Rules, Approvals and Review.",
            "Use the supplied prompts to adapt each section to a team you know (for example, a hotel front desk).",
            "Decide which data an agent may read, which it may change, and which changes need human approval.",
            "Name the accountable owner for the agent and for the data.",
        ],
        "produce": ["A one-page AI Data Governance Policy for your chosen team",
                    "A named accountable owner for at least one agent and one dataset"],
        "acceptance": "Your policy states the data an agent may touch, which actions need human approval, and who is accountable — a person, never the AI.",
    },
    {
        "n": 6, "topic": 3,
...
add("p", "After each activity, reflect using the same four themes. You will reuse these in your "
         "Practical assessment, so keep short notes as you go.")
